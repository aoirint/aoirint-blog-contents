...
- [Debug Mode - Official 7 Days to Die Wiki](https://7daystodie.fandom.com/wiki/Debug_Mode)

コンソールで`dm`を実行します。

Escメニューにデバッグツールが表示されるようになります。

まだ実績を解除していないスキル取得などをテストした場合、
デバッグモードを解除しないように注意してください。
デバッグモードを解除すると、条件を満たした実績が一括で解除されてしまうことがあります。

## クリエイティブモード

デバッグモードを有効化した後、コンソールで`cm`を実行します。

Uキーで任意のアイテムを取り出せるメニューが開くようになります。

## 経験値獲得

デバッグモードを有効化した後、コンソールで`giveselfxp 100000`を実行します。
100000 EXPを獲得します。
